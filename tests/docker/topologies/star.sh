...
RNS_PORT_BASE="${RNS_PORT_BASE:-8080}"
...
hub_dir="${OUT_DIR}/rns-hub"
gen_rns_config "$hub_dir" "TCPServerInterface:listen_ip=0.0.0.0:listen_port=4965"
gen_rns_service "$COMPOSE_FILE" "rns-hub" "$RNS_PORT_BASE" "$hub_dir"

ports=("RNS_HUB_PORT=${RNS_PORT_BASE}")
rns_ports=("$RNS_PORT_BASE")
lxmf_ports=()
client_services=()
volumes=("rns-hub-data")
...
for (( i=0; i<N; i++ )); do
  ...
  rns_port=$((RNS_PORT_BASE + 1 + i))
  lxmf_port=$((LXMF_PORT_BASE + i))
  config_dir="${OUT_DIR}/${rns_service}"

  gen_rns_config "$config_dir" \
    "TCPServerInterface:listen_ip=0.0.0.0:listen_port=4965" \
    "TCPClientInterface:target_host=rns-hub:target_port=4965"
  gen_rns_service "$COMPOSE_FILE" "$rns_service" "$rns_port" "$config_dir" "rns-hub"
